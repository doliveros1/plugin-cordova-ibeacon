...
public class LocationManager extends CordovaPlugin implements BeaconConsumer {
    ...
    private JSONObject mapOfBeacon(Beacon region) throws JSONException {
    	...
    	// Configurable en función del fabricante
        if(region.getDataFields() == null || region.getDataFields().size()==0){
            dict.put("battery", "No Information");
        } else{
            dict.put("battery", region.getDataFields().get(0));
        }
